...
from project1.settings import OUTPUT_DIR
...
def TD(lambda_val,
    ...
    plt.ylim((0, 0.85))
    plt.legend(loc=2)
    fig.savefig(os.path.join('..', 'output', OUTPUT_DIR, 'combined.png'))
    plt.close()
...
def plot_val_estimates(episode, alpha):
    ...
    plt.legend(loc=2)
    fig.savefig(os.path.join('..', 'output', OUTPUT_DIR, '{}.png'.format(str(episode).zfill(4))))
    plt.close()
